...
from datasets.DSEC.dataset.provider import DatasetProvider
...
def DSECEvents(dsec_dir, nr_events_data=1, delta_t_per_data=50, nr_events_window=-1,
    ...
    if mode == 'train':
        ...
        print("[DESC]: Found %s segmentation masks for split train" % (train_dataset.__len__()))
        return train_dataset
    else:
        val_dataset = dataset_provider.get_val_dataset()
        print("[DESC]: Found %s segmentation masks for split test" % (val_dataset.__len__()))
        return val_dataset
