...
try:
    ...
    from .csm_triton import CrossScan_channel, CrossMerge_channel
except:
    from csm_triton import cross_scan_fn, cross_merge_fn
    from csm_triton import CrossScan_channel, CrossMerge_channel
...
class Backbone_VSSM(VSSM):
    def __init__(self, 
                ...
                forward_type="v05_noz", mlp_ratio=4.0, 
    ...
    # def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln", **kwargs):
        ...
        super().__init__(
            ...
            ssm_d_state=ssm_d_state, ssm_ratio=ssm_ratio, ssm_dt_rank=ssm_dt_rank, ssm_conv=ssm_conv, ssm_conv_bias=ssm_conv_bias, forward_type=forward_type,
            mlp_ratio=mlp_ratio, norm_layer=norm_layer,
            downsample_version=downsample_version, patchembed_version=patchembed_version,
            use_checkpoint=use_checkpoint,
            **kwargs)  # 调用父类 VSSM 的初始化方法
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])
        _NORMLAYERS = dict(
            ln=nn.LayerNorm,
            ln2d=LayerNorm2d,
...
class SSMvc:
    def __initvc__(
        ...
        **kwargs,    
        ...
        factory_kwargs = {"device": None, "dtype": None}
        super().__init__()
        self.k_group = 2    # number of groups
        self.d_model = int(d_model)
        self.d_state = int(d_state)
        self.d_inner = int(ssm_ratio * d_model)
        self.d_ssm = int(d_ssm)
        self.dt_rank = int(math.ceil(self.d_ssm / 16) if dt_rank == "auto" else dt_rank)
        self.channel_first = channel_first
        self.with_dconv = d_conv > 1
        # Linear = Linear2d if channel_first else nn.Linear
        Linear = nn.Linear
        ...
        FORWARD_TYPES = dict(
            # ===============================
            vc=partial(self.forward_corev2, force_fp32=(not self.disable_force32), CrossScan=CrossScan_channel, CrossMerge=CrossMerge_channel),
        ...
        # x proj ============================
        self.x_proj = [
            nn.Linear(self.d_ssm, (self.dt_rank + self.d_state * 2), bias=False)
            for _ in range(self.k_group)
        ]
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, d_ssm)
        del self.x_proj
        ...
        if initialize in ["v0"]:
            self.A_logs, self.Ds, self.dt_projs_weight, self.dt_projs_bias = mamba_init.init_dt_A_D(
                self.d_state, self.dt_rank, self.d_ssm, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, k_group=self.k_group,
            )
        elif initialize in ["v1"]:
            # simple init dt_projs, A_logs, Ds
            self.Ds = nn.Parameter(torch.ones((self.k_group * self.d_inner)))
    ...
    def forward_corev2(
        ...
        no_einsum=False, # replace einsum with linear or conv1d to raise throughput
        # ==============================
        selective_scan_backend = None,  # 等价于SelectiveScanOflex,
        # CrossScan=CrossScan,
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            raise NotImplementedError("cascade2d not supported yet")
        else:
            x_proj_bias = getattr(self, "x_proj_bias", None)
            xs = CrossScan.apply(x) # (B, 2, H * W, C)
            if no_einsum:
                x_dbl = F.conv1d(xs.view(B, -1, L), self.x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K)
                dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2)
                if hasattr(self, "dt_projs_weight"):
                ...
                # print("before einsum shape:", xs.shape, self.x_proj_weight.shape)
                x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, self.x_proj_weight)
                if x_proj_bias is not None:
                    x_dbl = x_dbl + x_proj_bias.view(1, K, -1, 1)
                dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        B, C, H, W = x.shape
        x = x.flatten(2,3).permute(0, 2, 1).contiguous() # (b, h * w, c)
        x = self.in_proj(x) # (b, h * w, d_proj)
...
class Temporal_SSM(nn.Module, SSMvc):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
