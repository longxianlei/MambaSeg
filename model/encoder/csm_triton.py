...
# Channel Scan
class CrossScan_channel(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape
        ctx.shape = (B, C, H, W)
        xs = x.new_empty((B, 2, H * W, C))
        x = x.flatten(2, 3).transpose(dim0=1, dim1=2)
        xs[:, 0] = x
    ...
    def backward(ctx, ys: torch.Tensor):
        B, C, H, W = ctx.shape
        L = C
        ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]).view(B, -1, L)
        ys = ys.transpose(dim0=1, dim1=2)
        return ys.view(B, -1, H, W)

class CrossMerge_channel(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor):
        B, K, H, W, D = ys.shape
        ctx.shape = (H, W)
        ys = ys.view(B, K, -1, D)
        ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]).view(B, -1, D)
        ys = ys.transpose(dim0=1, dim1=2)
        return ys
    ...
    def backward(ctx, x: torch.Tensor):
        H, W = ctx.shape
        B, C, L = x.shape
        xs = x.new_empty((B, 2, L, C))
        x = x.transpose(dim0=1, dim1=2)
        xs[:, 0] = x
        xs[:, 1] = torch.flip(x, dims=[-1])
        xs = xs.view(B, 2, H, W, C)
        return xs
